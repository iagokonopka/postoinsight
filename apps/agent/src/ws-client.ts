...
import { extractFatoVenda, extractDespesa, extractDimProduto } from './extract.js'
...
export function connectLocation(location: LocationConfig): void {
  ...
  async function handleCommand(cmd: AgentCommand): Promise<void> {
    ...
    if (cmd.command === 'sync' && cmd.entity === 'despesa') {
      console.log(`[${location.sourceLocationId}] Sync despesa from ${cmd.watermark}`)
      let batch = 0
      let total = 0
      try {
        for await (const rows of extractDespesa({
          sourceLocationId: location.sourceLocationId,
          mode: 'sync',
          watermark: cmd.watermark,
          batchSize: BATCH_SIZE_SYNC,
    ...
    if (cmd.command === 'backfill') {
      ...
      console.log(`[${location.sourceLocationId}] Backfill ${entity} ${cmd.from} → ${cmd.to}`)
      const extractor = entity === 'despesa' ? extractDespesa : extractFatoVenda
      let batch = 0
      let total = 0
      try {

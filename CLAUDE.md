...
postoinsight/
  apps/
    web/                  ← Vite + React SPA (✅ em produção no Railway — web-production-c2e6bc.up.railway.app)
...
### ✅ `apps/web` — implementado e em produção (2026-05-25)
- Reimplementado do zero baseado em `docs/design/` e `design_example/postoinsight/`
- Deploy no Railway via Dockerfile multi-stage (node:20-alpine + nginx:alpine)
- Domínio: `web-production-c2e6bc.up.railway.app`
- `apps/web/nginx.conf` — SPA routing, cache headers, gzip
- `apps/web/Dockerfile` — multi-stage build com `VITE_API_URL` como build arg
- `apps/web/src/lib/api.ts` — helper `apiUrl()` usando `VITE_API_URL`
- Todos os hooks e páginas atualizados para usar `apiUrl()` (cross-origin Railway)
- Cookie de sessão alterado para `SameSite=None; Secure` em produção (necessário para cross-site entre subdomínios Railway)
- ADR-016 implementado
...
*Última atualização: 2026-05-25 — documento vivo, atualizado conforme o projeto evolui.*
